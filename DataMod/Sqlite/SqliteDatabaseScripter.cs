...
public static class SqliteDatabaseExtensions
{
    ...
    /// <returns></returns>
    public static List<Sql> ScriptIfNotExists(this Database database)
    {
...
"""));
    ...
    private static Sql ScriptAddColumnDefinition(Column column)
    {
        ...
        {
            Interpolate($"{Identifier(column.Name)} {column.StoreType} {(column.IsNullable ? "NULL" : "NOT NULL")}"),
    ...
    private static Sql ScriptColumnDefault(Column column)
    {
        if (!string.IsNullOrEmpty(column.DefaultValueSql))
        {
            ...
            else if (Regex.IsMatch(column.StoreType, "^[A-Za-z]+$"))
            {
                return Interpolate($" DEFAULT ''::{column.StoreType}");
            }
        }
...
CREATE INDEX {Identifier(index.GetName(change.TableName))} ON {Identifier(change.SchemaName, change.TableName)} ({Join(", ", index.Columns.Select(Identifier))});
""");
        }
        else
        {
            var constraint = change.Index;
            string type = constraint.IndexType == TableIndexType.PrimaryKey ? "PRIMARY KEY" : "UNIQUE";
            return Interpolate($"ALTER TABLE {Identifier(change.SchemaName, change.TableName)} ALTER CONSTRAINT {Identifier(constraint.GetName(change.TableName))} {type} ({Join(", ", constraint.Columns.Select(Identifier))});");
        }
    }
